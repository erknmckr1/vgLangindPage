...
export default function Step1_ProfileType() {
  ...
  const { profileType } = useSelector((state: RootState) => state.onBoarding);

  return (
    <div className="space-y-4">
      <h2 className="text-2xl font-semibold">Sizi daha yakından tanıyalım</h2>
      <p className="text-muted-foreground">
        İhtiyacınıza en uygun sistemi sunabilmemiz için kullanıcı tipinizi
        seçin.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
        {profileOptions.map((option) => (
          <button
            ...
            key={option.id}
            onClick={() =>
              dispatch(setField({ key: "profileType", value: option.id }))
            }
            className={cn(
              "border relative text-left rounded p-4 hover:bg-accent transition flex items-start gap-3",
              profileType === option.id && "border-primary bg-muted/70"
            )}
          >
            ...
            {profileType === option.id && (
              <CheckCircle2
                className="absolute bg-muted top-2 right-2 text-primary mt-1"
                size={20}
              />
            )}
          </button>
